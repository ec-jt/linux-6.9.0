...
/*
static void quirk_force_hotplug(struct pci_dev *pdev)
{
...
DECLARE_PCI_FIXUP_EARLY(PCI_VENDOR_ID_AMD,       0x1483, quirk_force_hotplug);
DECLARE_PCI_FIXUP_EARLY(PCI_VENDOR_ID_AMD,       0x1484, quirk_force_hotplug);
DECLARE_PCI_FIXUP_EARLY(PCI_VENDOR_ID_MICROSEMI, 0x4052, quirk_force_hotplug);

#define NP_64M  (64ULL << 20)

static bool is_pcie_bridge_port(struct pci_dev *pdev)
{
	if ((pdev->class >> 8) != PCI_CLASS_BRIDGE_PCI) return false;
	switch (pci_pcie_type(pdev)) {
	case PCI_EXP_TYPE_ROOT_PORT:
	case PCI_EXP_TYPE_UPSTREAM:
	case PCI_EXP_TYPE_DOWNSTREAM:
...
static void want_bridge_win(struct pci_dev *bridge, int which,
			    resource_size_t sz, unsigned long flags)
{
	struct resource *r = &bridge->resource[PCI_BRIDGE_RESOURCES + which];
	r->flags |= IORESOURCE_WINDOW | IORESOURCE_MEM | flags;
	if (resource_size(r) < sz) { r->start = 0; r->end = sz - 1; }
}

static void quirk_min_np_64m(struct pci_dev *pdev)
{
	if (!is_pcie_bridge_port(pdev)) return;
	want_bridge_win(pdev, 1, NP_64M, 0);
}
DECLARE_PCI_FIXUP_HEADER(PCI_VENDOR_ID_AMD,       0x1483, quirk_min_np_64m);
DECLARE_PCI_FIXUP_HEADER(PCI_VENDOR_ID_AMD,       0x1484, quirk_min_np_64m);
DECLARE_PCI_FIXUP_HEADER(PCI_VENDOR_ID_MICROSEMI, 0x4052, quirk_min_np_64m);

static void quirk_seed_pmem(struct pci_dev *pdev)
{
	if (!is_pcie_bridge_port(pdev)) return;
	bool up = (pci_pcie_type(pdev) == PCI_EXP_TYPE_UPSTREAM) ||
	          (pci_pcie_type(pdev) == PCI_EXP_TYPE_ROOT_PORT);
	resource_size_t sz = up ? (32ULL<<30) : (16ULL<<30);
	want_bridge_win(pdev, 2, sz, IORESOURCE_PREFETCH | IORESOURCE_MEM_64);
}
DECLARE_PCI_FIXUP_HEADER(PCI_VENDOR_ID_AMD,       0x1483, quirk_seed_pmem);
DECLARE_PCI_FIXUP_HEADER(PCI_VENDOR_ID_AMD,       0x1484, quirk_seed_pmem);
